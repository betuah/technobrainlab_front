        ...
        <v-card elevation="3" class="col-12 col-md-8 mx-auto pa-0 rounded-lg">
            ...
            >
                <template v-slot:[`item.code`]="{ item }">
                    ...
                    >
                        {{ item.paymentStats == '1' ? 'Proses' : ( item.paymentStats == '2' ? 'Terima' : 'Tolak') }}
                    </v-chip>
                </template>
                ...
                <template v-slot:[`item.email`]="{ item }">
                    <span class="font-weight-light text-caption">{{hideEmail(item.email)}}</span>
                </template>
    ...
    export default {
        ...
        methods: {
            async getData() {
                this.loading = true
                try {
                    const res  = await this.$axios.get(`/api/course/enroll/${process.env.NUXT_ENV_COURSE_META_2}`)
